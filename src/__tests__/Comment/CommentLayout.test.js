...
describe('<CommentLayout />', () => {
  ...
  describe('<Comment/>', () => {
    it('should render with correct comment prop', () => {
      expect(wrapper.find('Comment').prop('comment')).toEqual(
        defaultProps.comment.text,
      )
    })
  })
  ...
  describe('<div> for when user is not editing comment', () => {
    describe('when comment.editing is true', () => {
      it('should not render', () => {
        ...
        expect(wrapper.find({ 'data-testid': 'nonEditingDiv' })).toHaveLength(0)
      })
    })
    describe('when comment.editing is false', () => {
      it('should render', () => {
        wrapper = shallow(
          <CommentLayout {...defaultProps} comment={{ editing: false }} />,
        )
        expect(wrapper.find({ 'data-testid': 'nonEditingDiv' })).toHaveLength(1)
      })
    })
  ...
  describe('<div> for when user is editing comment', () => {
    describe('when comment.editing is true', () => {
      it('should render ', () => {
        ...
        expect(wrapper.find({ 'data-testid': 'editingDiv' })).toHaveLength(1)
      })
    })
    ...
    describe('when comment.editing is false', () => {
      it('should not render', () => {
        ...
        expect(wrapper.find({ 'data-testid': 'editingDiv' })).toHaveLength(0)
      })
    })
    ...
    describe('<input> for updating comment', () => {
      it('should render with defaultValue of current comment text', () => {
        wrapper = shallow(
          <CommentLayout
            {...defaultProps}
            comment={{ text: 'currentComment', editing: true }}
          />,
        )
        expect(
